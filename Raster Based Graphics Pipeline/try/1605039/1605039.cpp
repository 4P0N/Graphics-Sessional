...
int main()
{
    ...
    if (!conFile.is_open())
    {
        cout<<"conFile not found"<<endl;
    }
    ifstream projFile("stage3.txt");
    if (!projFile.is_open())
    {
        cout<<"projFile not found"<<endl;
    }
    ...
    for(int i=0;i<triangles.size();i++)
    {
        ...
        for(int j=topScanline;j<bottomScanline;j++)
        {
            ...
            double currLX=leftX+leftScanline*dx;
            double currRX=leftX+(rightScanline-1)*dx;

            double a1,b1,c1,a2,b2,c2,a,b,c,d;
            double leftZ,rightZ,inc;
            ...
            c=a1*b2-b1*a2;
            d=(-a*x1-b*y1-c*z1);
            leftZ=(-a/c)*currLX+(-b/c)*currY+(-d/c);
            rightZ=(-a/c)*currRX+(-b/c)*currY+(-d/c);
            inc=(rightZ-leftZ)/(rightScanline-leftScanline);

            int cnt=0;
            for (int k=leftScanline;k<rightScanline;k++) {
                double Z=leftZ+cnt*inc;
                cnt++;
    ...
    if (!bufFile.is_open()) {
        cout<< "bufferFile not found"<<endl;
    }
